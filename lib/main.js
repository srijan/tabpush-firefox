...
exports.main = function(options) {
  ...
  backgroundWorker.port.on("loginSucceeded", function(login) {
    loggedIn = true;
    loginName = login;
    if(optionsWorker) {
  ...
  popup.port.on("sendTab", function(dest) {
    if(loggedIn) {
      backgroundWorker.port.emit("sendTab", {"url":tabs.activeTab.url,"title":tabs.activeTab.title,"sender":loginName}, dest);
    }
    else {
